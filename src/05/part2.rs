...
fn draw_line(line: Line, mut map: HashMap<(i64, i64), i64>) -> HashMap<(i64, i64), i64> {
    if line.start.0 == line.end.0 {
    ...
    } else if i64::abs(line.start.0 - line.end.0) == i64::abs(line.start.1 - line.end.1) {
        // (diagonal)
        let mut sx = line.start.0;
        let mut ex = line.end.0;
        let mut sy = line.start.1;
        let mut dy = if line.start.1 < line.end.1 { 1 } else { -1 };
        ...
        for (i, x) in (sx..ex + 1).enumerate() {
            let k = (x, sy + i as i64 * dy);
            map.insert(k, map.get(&k).unwrap_or(&0) + 1);
        }
    } else {
        println!(
            "unknown {:?} {:?} {:?}",
            line,
            i64::abs(line.start.0 - line.start.1),
            i64::abs(line.end.0 - line.end.1)
        );
    }
...
mod tests {
    ...
    fn test_draw_diagonal() {
        ...
        let mut expected_map: HashMap<(i64, i64), i64> = HashMap::new();
        expected_map.insert((1, 1), 1);
        expected_map.insert((2, 2), 1);
        expected_map.insert((3, 3), 1);
        ...
        expected_map.insert((9, 7), 1);
        expected_map.insert((8, 8), 1);
        expected_map.insert((7, 9), 1);
...
5,5 -> 8,2",
        ...
        assert_eq!(count_overlapping_points(lines, map), 12);
    }
}
